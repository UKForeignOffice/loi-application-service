...
$(document).ready(function () {
  ...
  $('body')
    ...
    .on('click', '#document-search-back', function (e) {
      ...
      $.get('/select-documents?back=true&ajax=true', function (html) {
        $('.filtering').html(html)
        $.get('/get-last-search-ajax', {}).done(function (result) {
          if (result === null) {
            window.location = '/check-documents'
          } else {
            ...
            $('#sr-notification-container')
              .empty()
              .text('Back to previous search for ' + result + ', results can be found below.')
          }
        })
      })
    })
    ...
    .on('click', '.top-searches a', function (e) {
      ...
      $('#sr-notification-container')
        ...
        .text(
          'Your search for ' +
            this.href.substr(this.href.indexOf('=') + 1, this.href.length).replace(/%20/g, ' ') +
            ' has been completed, you can find results below',
        )
    ...
    .on('click', '.sortableHeader', function (e) {
      ...
      $.get(this.href + '&ajax=true', function (html) {
        ...
        $('#sr-notification-container')
          .empty()
          .text('You have sorted ' + column + ' in ' + order + ' order')
      })
    })
    .on(
      'click',
      'div.pager-controls > a, #dashboard-results > div.pager > div.pager-controls > ul > li > a',
      function (e) {
        e.preventDefault()
        ...
        $.get(this.href + '&ajax=true', function (html) {
          ...
          $('#sr-notification-container')
            .empty()
            .text('Results page has been changed to page ' + page)
        })
      },
    )
  ...
  $('#doc_search_button').click(function (e) {
    ...
    $('#sr-notification-container')
      .empty()
      .text('Your search for ' + $('#doc_search_field').val() + 'has been completed, you can find results below')
...
function updateDocumentCount(blur) {
  ...
  if (blur) {
    ...
    if (!remove) {
      ...
      $.post('/ajax-update-doc-count', JSON.parse(body), function (data, status) {
...
function removeLink(event, docId, source, searchTerm, sessionExpiry, doc_name) {
  ...
  $.get('/remove-document-ajax/' + docId + '?searchTerm=' + searchTerm + '&source=' + source, function (html) {
    $('#sr-notification-container')
      .empty()
      .text(doc_name + ' has been removed')
    $('#shopping_basket_container').html(html)
...
function ajaxSearch(search_term) {
  ...
  $.get('/select-documents', { searchTerm: decodeURI(search_term), ajax: true }).done(function (html) {
    $('.filtering').html(html)
