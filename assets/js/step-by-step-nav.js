...
(function (Modules) {
    ...
    Modules.AppStepNav = function () {
        ...
        this.start = function ($element) {
            ...
            stepNavSize = $element.hasClass('app-step-nav--large')
                ? 'Big'
                : 'Small';
            rememberShownStep =
                !!$element.filter('[data-remember]').length &&
                stepNavSize === 'Big';
            ...
            var stepNavTracker = new StepNavTracker(
                totalSteps,
                totalLinks,
                uniqueId
            );
            ...
            function headerIsOpen($stepHeader) {
                return (
                    typeof $stepHeader.closest('.js-step').data('show') !==
                    'undefined'
                );
            }
            ...
            function addAriaControlsAttrForShowHideAllButton() {
                ...
                $showOrHideAllButton = $element.find(
            ...
            function bindToggleForSteps(stepNavTracker) {
                $element.find('.js-toggle-panel').click(function (event) {
                    ...
                    var stepIsOptional =
                        typeof $step.data('optional') !== 'undefined';
            ...
            function rememberStepState($step) {
                if (rememberShownStep) {
                    var data =
                        JSON.parse(loadFromSessionStorage(uniqueId)) || [];
            ...
            function ensureOnlyOneActiveLink() {
                var $activeLinks = $element.find(
                    '.js-list-item.' + activeLinkClass
                );
