  ...
  if (error === 'Enter your address manually instead') {
    if (url.includes('alternative')) {
      var html =
        ...
        '<ul class="govuk-list govuk-error-summary__list"><li style="color: #d4351c"><strong><a href="/your-alternative-address-manual">' +
        error +
        '</a></strong></li></ul>'
      ...
      var html =
        '<h2 class="govuk-error-summary__title" id="error-summary-heading">Postcode search is not available at the moment</h2>' +
        '<ul class="govuk-list govuk-error-summary__list"><li style="color: #d4351c"><strong><a href="/your-main-address-manual">' +
        error +
        '</a></strong></li></ul>'
      $('#postcode-error').removeClass('hide').html(html)
    }
    ...
    var html =
      '<h2 class="govuk-error-summary__title" id="error-summary-heading" autofocus>There is a problem</h2>' +
      '<ul class="govuk-list govuk-error-summary__list"><li style="color: #d4351c"><strong>' +
      error +
      '</strong></li></ul>'
    $('#postcode-error').removeClass('hide').html(html)
  }
...
$(document).on('change blur keydown paste input', '.number', function () {
  ...
  if (isPremium) {
    var cost =
      totalCost < 0 || sum % 1 !== 0 || sum > 20 ? 'Maximum 20 documents per application' : '£' + totalCost.toString()
  } else {
    var cost =
      totalCost < 0 || sum % 1 !== 0 || sum > 100 ? 'Maximum 100 documents per application' : '£' + totalCost.toString()
  }

  $('#cost').text(cost)
  $('#sr-notification-container')
    .empty()
    .text('Cost total updated, new total is ' + cost)
})
