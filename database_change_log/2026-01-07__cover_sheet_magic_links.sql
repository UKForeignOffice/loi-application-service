-- FUNCTION: public.dashboard_data_eapp(integer, integer, integer, text, text, text, text, text)

DROP FUNCTION IF EXISTS public.dashboard_data_eapp(integer, integer, integer, text, text, text, text, text);

CREATE OR REPLACE FUNCTION public.dashboard_data_eapp(
	_user_id integer,
	_limit integer,
	_offset integer,
	_orderby text,
    ...
    RETURNS TABLE("createdDate" timestamp with time zone, unique_app_id text, applicationtype text, doc_count integer, payment_amount numeric, user_ref text, main_postcode text, result_count integer, submission_destination text, submitted text, application_guid text)
    LANGUAGE 'plpgsql'
    COST 100
