...
async function checkFileType(req) {
  try {
    ...
    for (const fileFromSession of checkedFilesFromSession) {
      inDevEnvironment ? await checkLocalFileType(fileFromSession, req) : await checkS3FileType(fileFromSession, req)
  ...
  try {
    ...
    for (const fileFromSession of uploadedFileData) {
      ...
      if (digitalSignature.length > 0) {
        ...
        sails.log.info(`${fileFromSession.filename} does not contain a digital signature`)
      }
    }
  } catch (err) {
...
function checkLocalFileSignature(file, req) {
  try {
    ...
    if (!signature) {
      throw new PdfSignatureError('cannot find signature')
...
function addErrorToSessionIfNotPDF(file, req, fileType) {
  if (!fileType || fileType.mime !== 'application/pdf') {
    ...
    removeSingleFile(req, file)
    const error = ['Wrong file type. Only PDF files are allowed']
...
function addErrorToSessionIfNoSignatureExists(file, req) {
  const error = ['No digital signature detected in PDF']
  req.flash('displayFilenameErrors', [{ filename: file.filename, errors: error }])
}
...
async function virusScan(req) {
  ...
  try {
    ...
    for (const fileFromSession of checkedFilesFromSession) {
      inDevEnvironment ? await scanFilesLocally(fileFromSession, req) : await scanStreamOfS3File(fileFromSession, req)
    }
  } catch (err) {
...
async function scanStreamOfS3File(file, req) {
  try {
    const s3Bucket = req._sails.config.upload.s3_bucket
    const scanResults = await clamscan.scanStream(await getS3FileStream(file.storageName, s3Bucket))
...
async function addCleanAndUnsubmittedTagsToFile(file, req) {
  try {
    ...
    sails.log.info(`Both CLEAN and UNSUBMITTED tags added to ${file.storageName}`)
  } catch (err) {
...
function checkTypeAndDuplication(req, file, cb) {
  ...
  const fileAlreadyExists = uploadedFileData?.find((existing) => existing.filename === file.originalname) ?? null
...
function removeFilesIfLarge(req) {
  const UPLOAD_LIMIT_TO_MB = req._sails.config.upload.file_upload_size_limit * 1_000_000
  ...
  for (const file of req.files) {
    ...
    if (file.size > UPLOAD_LIMIT_TO_MB) {
      const error = [`File is too big. Each file must be a maximum of ${formatFileSizeMb(UPLOAD_LIMIT_TO_MB, 0)}`]
      req.flash('displayFilenameErrors', [{ filename: file.originalname, errors: error }])
      removeSingleFile(req, file)
    }
  }
}
