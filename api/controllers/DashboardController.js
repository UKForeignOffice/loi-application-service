...
const DashboardController = {
    ...
    Application.count({
      ...
      .then((totalApplications) => {
        return HelperService.refreshUserData(req, res).then(() => {
          ...
          const { storedProcedureArgs, displayAppsArgs } = DashboardController._calculateSortParams(
            req,
            res,
            userData,
            totalApplications,
  ...
  _calculateSortParams(req, res, userData, totalApplications) {
    ...
    const searchCriteria = req.allParams().dashboardFilter || req.query.searchText || ''
  ...
  _getApplications(storedProcedureArgs, displayAppsArgs) {
    const queryApplications = DashboardController._chooseStoredProcedure(storedProcedureArgs.secondarySortOrder)
  ...
  async _displayApplications(results, displayAppsArgs) {
    try {
      ...
      return DashboardController._addStatusesToApplications(orbitData, {
        ...displayAppsArgs,
        results,
      })
    } catch (error) {
  ...
  _addStatusesToApplications(apiResponse, displayAppsArgs) {
    const { userData, totalApplications, offset, sortOrder, currentPage, searchCriteria, pageSize, req, res, results } =
      displayAppsArgs
  ...
  _paginationAndPageTotal(results, offset, pageSize) {
    ...
    const totalPages = resultCount % pageSize === 0 ? resultCount / pageSize : Math.floor(resultCount / pageSize) + 1
