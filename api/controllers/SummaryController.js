      ...
      async.series(
        ...
        (err, results) => {
          ...
          if (printable) {
            sequelize
              .query(
                'SELECT DISTINCT application_id, payment_complete, payment_amount, payment_reference,id, "createdAt", "updatedAt", payment_status, oneclick_reference FROM "ApplicationPaymentDetails" WHERE application_id=' +
                  req.session.appId,
                { type: sequelize.QueryTypes.SELECT },
              )
