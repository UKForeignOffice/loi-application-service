...
const OpenEAppController = {
  async renderPage(req, res) {
    ...
    try {
      ...
      const caseManagementResponse = await OpenEAppController._getApplicationDataFromOrbit(req, res)
      ...
      const daysLeftToDownload =
        caseManagementStatus === 'Completed'
          ? OpenEAppController._calculateDaysLeftToDownload(caseManagementData, req)
          : 0
      const applicationExpired = OpenEAppController._hasApplicationExpired(caseManagementData, daysLeftToDownload)

      const noOfRejectedDocs = OpenEAppController._calculateRejectedDocs(caseManagementData)
  ...
  _formatDataForPage(applicationTableData, caseManagementResponse) {
    ...
    if (!caseManagementResponse.payment.transactions || caseManagementResponse.payment.transactions.length === 0) {
    ...
    return {
      ...
      dateCompleted:
        caseManagementResponse.completedDate === null
          ? 'N/A'
          : OpenEAppController._formatDate(caseManagementResponse.completedDate),
      documents: caseManagementResponse.documents,
      originalCost: HelperService.formatToUKCurrency(caseManagementResponse.payment.transactions[0].amount || 0),
  ...
  _generateOrbitReceiptUrl(applicationRef, storageLocation, config) {
    ...
    return promise.then(
      (url) => {
        sails.log.info(`Presigned url generated for ${applicationRef} receipt`)
        return url
      },
      (err) => {
        throw new Error(err)
  ...
  async _streamOrbitReceiptToClient(req, res) {
    try {
      ...
      const { orbit_bucket: s3Bucket, orbit_url_expiry_hours: s3UrlExpiryHours } = req._sails.config.upload

      const storageLocation = Buffer.from(req.params.storageLocation, 'base64').toString()

      const url = await OpenEAppController._generateOrbitReceiptUrl(req.params.applicationRef, storageLocation, {
        s3Bucket,
        s3UrlExpiryHours,
      })

      if (!url) {
        console.error(`Unable to generate pre-signed url for ${req.params.applicationRef} receipt`)
      }
