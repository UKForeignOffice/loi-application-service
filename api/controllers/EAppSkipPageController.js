const EAppSkipPageController = {
  ...
  handleChoice(req, res) {
    ...
    if (suitabilityQuestionsSkipped) {
      return res.redirect(EAppSkipPageController._skippedQuestionsUrl(req))
    }
  ...
  _skippedQuestionsUrl(req) {
    ...
    return userLoggedIn ? '/upload-files' : `${userServiceURL}/sign-in?next=continueEApp&from=start`
  },
}
