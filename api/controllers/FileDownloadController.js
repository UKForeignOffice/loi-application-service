...
const FileDownloadController = {
  async downloadFileHandler(req, res) {
    try {
      const apostilleRefBelongstoApp = await FileDownloadController._apostilleRefBelongToApplication(req, res)

      if (!apostilleRefBelongstoApp) {
        sails.log.error('Apostille ref does not belong to this application')
  ...
  _apostilleRefBelongToApplication(req, res) {
    ...
    return OrbitService.getApplicationStatusFromOrbit([applicationRef])
      .then((response) => {
        ...
        if (appInfo) {
          const apostilleRefs = appInfo.documents.map((document) => document.apostilleReference)
  ...
  async _streamOrbitFileToClient(req, res) {
    try {
      ...
      const url = await FileDownloadController._generateOrbitApostilleUrl(
        apostilleReference,
