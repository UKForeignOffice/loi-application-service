...
const FileUploadController = {
  ...
  async uploadFilesPage(req, res) {
    try {
      ...
      const noUploadFileDataExistsInSession = !req.session?.eApp?.uploadedFileData
  ...
  async _addSignedInDetailsToApplication(req, res) {
    try {
      ...
      const appHasCorrectSignInID = currentApplicationFromDB.dataValues.user_id !== PRE_SIGNED_IN_USER_ID

      currentApplicationFromDB.update({
        feedback_consent: feedbackConsent,
      })

      sails.log.info(`feedback_consent has been updated to ${account.feedback_consent} for application_id ${appId}`)
  ...
  _addS3LocationToSession(req) {
    ...
    uploadedFileData.forEach((uploadedFile) => {
      const newFileUploadData = req.files.find((file) => uploadedFile.filename === file.originalname)
  ...
  deleteFileHandler(req, res) {
    ...
    req.session.eApp.uploadedFileData = FileUploadController._removeFileFromSessionArray(req, uploadedFileData)
    FileUploadController._redirectToUploadPage(res)
  },
