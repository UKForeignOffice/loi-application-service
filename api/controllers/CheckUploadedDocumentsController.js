...
const CheckUploadedDocumentsController = {
  async renderPage(req, res) {
    if (HelperService.maxFileLimitExceeded(req)) return res.serverError('maxFileLimitExceeded')
  ...
  _checkDocumentCountAndPaymentDetails(req, res) {
    ...
    CheckUploadedDocumentsController._checkAdditionalApplicationInfoInDB(req, res)
  },
  ...
  _checkDocumentCountInDB(req, res, params) {
    UserDocumentCount.findOne({
      ...
      if (!data) {
        CheckUploadedDocumentsController._createDocumentCountInDB(req, res, params)
      } else {
        CheckUploadedDocumentsController._updateDocumentCountInDB(req, res, params)
      }
    })
  },
  ...
  _checkPaymentDetailsExistsInDB(req, res, params) {
    return ApplicationPaymentDetails.findOne({
      ...
      if (!data) {
        CheckUploadedDocumentsController._createPaymentDetailsInDB(params, res)
      } else {
        ...
        CheckUploadedDocumentsController._updatePaymentAmountInDB(params, res)
      }
    })
  },
  ...
  _createPaymentDetailsInDB(params, res) {
    return ApplicationPaymentDetails.create({
      ...
      .then(() => {
        sails.log.info(`Payment details added to db for application ${params.appId}`)
  ...
  _updatePaymentAmountInDB(params, res) {
    return ApplicationPaymentDetails.update(
      ...
      .then(() => {
        sails.log.info(`Payment details updated in db for application ${params.appId}`)
        res.redirect(307, params.redirectUrl)
      })
      .catch((error) => {
        sails.log.error(error)
