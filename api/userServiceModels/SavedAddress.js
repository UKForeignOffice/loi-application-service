module.exports = function(sequelize, DataTypes) {

  return sequelize.define('SavedAddress', {
    id: {
      type: DataTypes.INTEGER,
      allowNull: false,
      primaryKey: true,
      autoIncrement: true,
      _autoGenerated: true
    },
    user_id: {type: 'integer', allowNull: false},

    full_name: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        len: {
          args: [2, 250],
          msg: JSON.stringify([{
            "errInfo": 'You have not provided your full name',
            "errSoltn": 'Please enter your full name',
            "questionId": 'full_name'
          }])
        }
      }
    },
    postcode: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        len: {
          args: [1],
          msg: JSON.stringify([{
            "errInfo": 'You have not provided a valid postcode',
            "errSoltn": 'Please enter a valid postcode',
            "questionId": 'postcode'
          }])
        }
      }
    },
    organisation: {
      type: DataTypes.STRING,
      allowNull: true
    },
    house_name: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        notEmpty: {
          msg: JSON.stringify([{
            "errInfo": 'You have not provided the house name or number of your address',
            "errSoltn": 'Please enter the house name or number of your address',
            "questionId": 'house_name'
          }])
        }
      }
    },
    street: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        notEmpty: {
          msg: JSON.stringify([{
            "errInfo": 'You have not provided the street of your address',
            "errSoltn": 'Please enter your street',
            "questionId": 'street'
          }])
        }
      }

    },

    town: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        notEmpty: {
          msg: JSON.stringify([{
            "errInfo": 'You have not provided a town',
            "errSoltn": 'Please enter the town',
            "questionId": 'town'
          }])
        }
      }
    },
    county: {
      type: DataTypes.STRING,
      allowNull: true

    },
    country: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        notEmpty: {
          msg: JSON.stringify([{
            "errInfo": 'You have not provided a country',
            "errSoltn": 'Please enter the country',
            "questionId": 'country'
          }])
        }
      }
    },
    telephone: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        len: {
          args: [6, 25], //
          msg: JSON.stringify([{
            "errInfo": 'You have not provided a valid phone number',
            "errSoltn": 'Enter a valid phone number',
            "questionId": 'telephone'
          }])
        }
      }
    },
    mobileNo: {
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        len: {
          args: [6, 25], //
          msg: JSON.stringify([{
            "errInfo": 'You have not provided a valid mobile number',
            "errSoltn": 'Enter a valid mobile number',
            "questionId": 'mobileNo'
          }])
        }
      }
    },
    email: {
      type: DataTypes.STRING,
      allowNull: true,
      validate: {
        isEmail: {
          msg: JSON.stringify([{
            "errInfo": 'The email address you have entered is invalid',
            "errSoltn": 'Enter a valid email address',
            "questionId": 'email'
          }])
        },
        notIn: {
          args: [['INVALID']],
          msg: JSON.stringify([{
            "errInfo": 'The email address you have entered is invalid',
            "errSoltn": 'Enter a valid email address',
            "questionId": 'email'
          }])
        }
      }
    }


  })
}

