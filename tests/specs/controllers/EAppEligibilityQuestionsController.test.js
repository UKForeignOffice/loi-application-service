...
describe('EAppEligibilityQuestionsController', () => {
  ...
  describe('renderPage functions', () => {
    it('should render the correct ejs pages based on the url param', () => {
      ...
      expect(resStub.view.getCall(0).args[0]).to.equal('eApostilles/eligibilityQuestionOne.ejs')
      expect(resStub.view.getCall(1).args[0]).to.equal('eApostilles/eligibilityQuestionTwo.ejs')
      expect(resStub.view.getCall(2).args[0]).to.equal('eApostilles/eligibilityQuestionThree.ejs')
    })
  })
  ...
  describe('handle answer functions', () => {
    ...
    it('should redirect to the correct path whene user selects YES radio button', () => {
      ...
      expect(resStub.redirect.getCall(0).args[0]).to.equal('/eligibility/check-recipient-accepts-eapostilles')
      expect(resStub.redirect.getCall(1).args[0]).to.equal('/eligibility/check-documents-are-prepared')
      expect(resStub.redirect.getCall(2).args[0]).to.equal('/completing-your-application')
    })
    ...
    it('should redirect to the correct path whene user selects NO radio button', () => {
      ...
      // then
      expect(resStub.redirect.callCount).to.equal(3)
      expect(resStub.redirect.getCall(0).args[0]).to.equal('/exit-pages/you-cannot-apply-yet')
      expect(resStub.redirect.getCall(1).args[0]).to.equal('/exit-pages/check-recipient-accepts-eapostilles-exit')
      expect(resStub.redirect.getCall(2).args[0]).to.equal('/exit-pages/use-paper-based-service')
    })
