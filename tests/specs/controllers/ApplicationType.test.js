   ...
   describe.skip('[FUNCTION: serviceSelectorPage()]', function() {
       it('should load the the Service Selector page and populate session vars with empty data if a new application, or current data if an application update ', function(done) {
               User.findOne({ where: { email: testEmail } }).then(function(user) {
                   AccountDetails.findOne({ where: { user_id: user.id } })
                   .then(function(account) {
                       ...
                       chai.assert.isOk(sails.config.customURLs.userServiceURL == 'http://localhost:8080/api/user', "Successfully found Service URL");
       ...
       it('should find the applicationType tempate view', function(done) {
           ...
           fs.stat('views/applicationForms/applicationType.ejs', function(err, stat) {
               if (err === null) {
                   ...
                   chai.assert.isNotOk(err, 'Failed to find applicationType template');
                   done(err);
               }
           });
       });
       ...
       it('should use a generated a unique applicationId and successfully check it is unique, ', function(done) {
           ...
           .then(function (data) {
               if (data !==null) {
                   ...
                   sequelize.query('SELECT unique_app_id FROM "Application" WHERE unique_app_id = \'' + uniqueApplicationId + '\';')
                    .spread(function (result, metadata) {
                       if (result.length !== 0) {
                           ...
                           chai.assert.isOk(result.length === 0, 'Successfully found unique application reference.');
                   ...
                   chai.assert.isNotOk(data, 'Failed to retrieve most current ApplicationReference for this new application');
                    done();
               }
           });
       });
