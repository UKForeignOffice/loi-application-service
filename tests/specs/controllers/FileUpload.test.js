...
describe('uploadFilesPage', () => {
  ...
  it('should load uploadedFiles.ejs with user_data', async () => {
    ...
    sandbox.stub(HelperService, 'getUserData').callsFake(() => testUserData)
    sandbox.stub(NodeClam.prototype, 'init').resolves()
    ...
    // then
    expect(resStub.view.firstCall.args[0]).to.equal('eApostilles/uploadFiles.ejs')
  ...
  it('should log error if not connected to clamAV', async () => {
    ...
    sandbox.stub(NodeClam.prototype, 'init').rejects('Turned off for testing.')
    await FileUploadController.uploadFilesPage(reqStub, resStub)
...
describe('uploadFileHandler', () => {
  ...
  it('triggers noFileUploadedError if no files uploaded', async () => {
    ...
    expect(reqStub.flash.firstCall.args[1]).to.deep.equal(['No files have been selected'])
  })
