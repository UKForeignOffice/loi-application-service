...
describe('EAppSubmittedController', () => {
  ...
  describe('addDocsAndRenderPage', () => {
    beforeEach(() => {
      s3Mock.on(GetObjectCommand).resolves('test_file_url')
    })
    ...
    it('should upload files to the database if they exist', async () => {
      // when
      const createUploadedDocumentsUrls = sandbox.stub(UploadedDocumentUrls, 'create')
      ...
      expect(createUploadedDocumentsUrls.getCall(0).args[0]).to.deep.equal(firstCallArgs)
      expect(createUploadedDocumentsUrls.getCall(1).args[0]).to.deep.equal(secondCallArgs)
    })
  })
  ...
  describe('_renderPageAndSendConfirmationEmail', () => {
    ...
    beforeEach(async () => {
      sandbox.stub(HelperService, 'getUserData').callsFake(() => stubUserData)
      sandbox.stub(UploadedDocumentUrls, 'create').resolves()
      emailSubmission = sandbox.stub(EmailService, 'submissionConfirmation').callsFake(() => null)
      await EAppSubmittedController.addDocsAndRenderPage(reqStub, resStub)
    })
    ...
    it('should render submission page', () => {
      ...
      expect(resStub.view.calledWith('eApostilles/applicationSubmissionSuccessful.ejs', expectedArgs)).to.be.true

      expect(resStub.view.getCall(0).args[0]).to.equal('eApostilles/applicationSubmissionSuccessful.ejs')
