...
describe('EAppSkipPageController', () => {
  ...
  it('shows the page when renderPage() is called', () => {
    ...
    // then
    expect(resStub.view.getCall(0).args[0]).to.equal('eApostilles/eAppSkipPage.ejs')
  ...
  it('redirects to suitability questions if YES radio selected', () => {
    ...
    // then
    expect(resStub.redirect.getCall(0).args[0]).to.equal('/eligibility/check-documents-are-eligible')
  })
  ...
  it('redirects to sign in page if NO radio selected and user is NOT logged in', () => {
    ...
    // then
    expect(resStub.redirect.getCall(0).args[0]).to.equal('test.com/sign-in?next=continueEApp&from=start')
  })
