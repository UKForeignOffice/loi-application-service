...
describe('OpenEAppController', () => {
  ...
  describe('happy path', () => {
    ...
    beforeEach(() => {
      ...
      findApplicationData = sandbox.stub(Application, 'findOne').resolves(resolvedAppData)
      sandbox.stub(OpenEAppController, '_getApplicationDataFromOrbit').resolves(resolvedOrbitData)
  ...
  describe('date countdown', () => {
    it('shows correct number of days for 11 day old application', async () => {
      ...
      sandbox.stub(Date, 'now').callsFake(() => TWELVE_DAYS_AFTER_COMPLETION)
      await OpenEAppController.renderPage(reqStub, resStub)
  ...
  describe('allDocumentsRejected', () => {
    ...
    it('returns false if no documents rejected', async () => {
      ...
      sandbox.stub(OpenEAppController, '_getApplicationDataFromOrbit').resolves(updatedCasebookData)
      await OpenEAppController.renderPage(reqStub, resStub)
