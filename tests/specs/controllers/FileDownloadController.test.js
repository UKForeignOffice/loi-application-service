...
describe('FileDownloadController', () => {
  ...
  it('throws if user is not logged in', () => {
    // when
    sandbox.stub(HelperService, 'getUserData').callsFake(() => ({ loggedIn: false }))
  ...
  it('throws if unique_app_id is not found', () => {
    ...
    sandbox.stub(HelperService, 'getUserData').callsFake(() => ({ loggedIn: true }))
    const fn = () => FileDownloadController._urlErrorChecks(reqStub, resStub)
  ...
  describe('_apostilleRefBelongToApplication', () => {
    ...
    it('sends the correct argument to OrbitService', () => {
      // when
      const getApplicationStub = sandbox.stub(OrbitService, 'getApplicationStatusFromOrbit').resolves([
        {
          documents: [],
        },
      ])
      ...
      // then
      expect(getApplicationStub.getCall(0).args[0]).to.eql([reqStub.params.unique_app_id])
    })
