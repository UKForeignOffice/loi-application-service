...
describe('DocumentQuantityController', () => {
  ...
  describe('[Function: userDocumentQuantityPage] #STANDARD- Run userDocumentQuantityPage()', () => {
    it('should return user doc count for current application, if found populate the form and allow an update, else leave form empty and allow a create to occur', async () => {
      vi.spyOn(sequelize, 'query')
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([{ doc_id: 1 }])

      /* cause query to return nothing */
      const noResults = await sequelize.query('select doc_id from "UserDocuments" where application_id=100000000000', {
        type: sequelize.QueryTypes.SELECT,
      })
      assert.equal(noResults.length, 0)

      /* cause query to return something */
      const someResults = await sequelize.query(
        `select doc_id from "UserDocuments" where application_id='${standard_applicationID}'`,
        {
          type: sequelize.QueryTypes.SELECT,
        },
      )
      assert.isAbove(someResults.length, 0)
    })
  })
  ...
  //                    assert.ok('A user document count was found, meaning the form will be populated with previous data and an UPDATE will be carried out');
  ...
  //                    assert.ok('No user document counts were found, meaning the form will empty and a CREATE will be carried out');
  ...
  //                    )
  //                        .then(function() {
  //                            assert.notOk('The insert data is invalid so errors should be yielded, so you should NOT see this message.');
  //                        })
  //                        .catch(SequelizeValidationError, function(error) {
  //                            assert.ok('The update data is invalid, errors have been yielded. ', error);
  //                        });
  //                }
  //
  //                return null;
  ...
  //            .then(function (data) {
  //                if (selectedDocsCount.length<1) {
  //                    assert.ok('No user document counts were found, meaning the form will empty and a CREATE will be carried out, but there insert data is invalid so errors should be yielded');
  //                    UserDocumentCount.create({
  //                        application_id: standard_applicationID,
  //                        doc_count: x, // invalid character
  //                        price: xx // invalid characters
  ...
  //                        })
  //                        .catch(SequelizeValidationError, function(error) {
  //                            assert.ok('The insert data is invalid, errors have been yielded. ', error);
  //                        });
  //                }
  //
  //                return null;
